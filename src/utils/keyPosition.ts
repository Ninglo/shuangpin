...
const createPhoneme = ({ isParagraphCourse, nextLead, config, menuIndex, _mode }: Params) => (char: string, i: number): Phoneme => {
...
export function createWholeSeq(chars: readonly string[], config: ShuangpinConfig, menuIndex: number, _mode?: 'leads' | 'follows'): Phoneme[] {
    ...
    return chars.map<Phoneme>(createPhoneme({ isParagraphCourse, nextLead, config, menuIndex, _mode }));
}
